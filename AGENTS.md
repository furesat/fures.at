...
### zuzumood – Etsy Destekli Butik Mağaza Deneyimi
- **Amaç:** `www.zuzumood.com` için Texas merkezli marka diliyle uyumlu, Etsy fulfillment odaklı butik e‑ticaret vitrini oluşturmak ve sürdürmek.
...
- **İçerik Notları:**
  ...
  - Header'da PDF indirme/print butonu `t.actions.downloadPdf` etiketini kullanır ve `window.print()` çağrısı ile çalışır; header `no-print` sınıfıyla çıktıda görünmez.
  - Almanca dahil hiçbir dilde "Mehr", "Mehr entdecken", "Erzählen Sie uns von Ihrem Projekt", "Angetrieben von Fures" kalıpları yer almamalı; chatbot selamlamaları sade tutulmalı.
...
## Güncelleme Notu (2026-06-08 / Maria Alm Linkleri ve Dinamik Sitemap)
- `src/components/FooterDE.tsx` alt barına `fures.at` bilgisinden sonra `ein Projekt von Tourismus Förder Verband` dış bağlantısı eklendi; bağlantı `https://tourismusverband.net` adresine `noopener noreferrer` güvenliğiyle açılır.
- `src/components/Projects.tsx` içindeki `maria-alm-route-atlas` referans kartının canlı linki Netlify ön izlemesinden `https://inmariaalm.at` alan adına taşındı; dış link davranışı ve çok dilli yapım aşaması açıklamaları korundu.
- Depoda önceden `sitemap*.xml` dosyası bulunmadığı için `src/sitemap.xml.njk` ile üretim sırasında `public/sitemap.xml` oluşturan dinamik sitemap şablonu ve `.eleventy.js` içinde `sitemapDate` filtresi eklendi. Statik dil rotaları, profil/araç sayfaları, statik proje vaka sayfaları, tüm blog yazıları ve tüm kampanya yazıları sitemap'e dahil edilir; yalnızca dış `inmariaalm.at` linki sitemap'e eklenmez.
- Blog ve kampanya sitemap kapsamı günlük otomasyonla geride kalmasın diye `src/_data/blogPosts.js` ve `src/_data/campaignPosts.js` içindeki `all` listeleri artık son 50 kayıtla sınırlanmaz; RSS feed listeleri yine `MAX_ITEMS` limitiyle korunur; front matter parse uyarısı veren eski bloglarda dil bilgisi klasör adından yedeklenir. 2025-12-03 tarihli TR/DE bloglarda çok satırlı `description` front matter blok scalar formatına düzeltilerek Eleventy sitemap üretimi temizlendi.

## Güncelleme Notu (2026-06-12 / İletişim Formu Teslimatı)
- `src/components/ContactForm.tsx` ile ana iletişim akışına çok dilli, validasyonlu form eklendi; form birincil olarak FormSubmit AJAX üzerinden `furkanyonat@gmail.com` adresine iletir ve aynı gönderimi Netlify Forms `fures-contact` yedek kaydına da post eder.
- `index.html` içine Netlify'nin build sırasında formu algılayabilmesi için gizli `fures-contact` form tanımı eklendi; Netlify panelinde bu form için e-posta bildirimi gerekiyorsa hedef adres `furkanyonat@gmail.com` olarak doğrulanmalıdır.
- `src/components/CTA.tsx` ve `src/pages/de/ContactPageDE.tsx` iletişim sayfaları artık mailto butonu yerine gerçek formu gösterir; Calendly toplantı bağlantısı ve mevcut SEO/canonical rotalar korunur.
- `src/contexts/LanguageContext.tsx` içindeki TR/EN/DE/RU çeviri setlerine form alanları, başarı ve hata mesajları eklendi; yeni rota oluşturulmadığı için sitemap şablonuna ek URL gerekmez.
