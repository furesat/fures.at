...
## Güncelleme Notu (2026-06-12 / Netlify Forms İletişim Akışı)
- Ana site iletişim akışı Netlify Forms uyumlu hale getirildi: `fures-contact` adlı form `index.html` içinde statik olarak tanıtılır ve React tarafında aynı form adıyla gönderim yapılır.
- Görünür iletişim formu `src/components/NetlifyContactForm.tsx` içinde merkezi tutulur; TR/EN/RU iletişim sayfaları `CTA` bileşeni üzerinden, Almanca `/de/kontakt` sayfası ise aynı bileşeni doğrudan kullanır.
- Form alanları `name`, `email`, `company`, `start-date`, `message`, `language` ve `recipient` olarak korunmalı; `recipient` değeri `furkanyonat@gmail.com` kalmalıdır. Netlify panelinde Form Notifications kısmında aynı e-posta adresine bildirim tanımlandığını deploy sonrası doğrula.
- Başarılı gönderimde sayfa içinde teşekkür ekranı gösterilir; yeni rota eklenmediği için sitemap şablonunda ek URL gerekmez. Deploy öncesinde `public/sitemap.xml` üretimini kontrol etmeye devam et.
